...
file_id <- random_note_04[,5]
colnames(file_id) <- c("url")
file_id$url <- gsub("https://drive.google.com/open\\?id=", "", file_id$url)

## create folder to download pdf files
dir.create("./note_04_random")
dir.create("./note_04_random/pdf")

## download files
for (i in 1:length(file_id$url)) {
  drive_download(file = as_id(file_id$url[i]), path = paste0("./note_04_random/pdf/","note_04_random-",i,".pdf"), overwrite = TRUE)
}

## create folder to convert files
dir.create("./note_04_random/jpg")

pdf_file <- paste0("./note_04_random/pdf/note_04_random-", seq(1,5,1),".pdf")
jpg_file <- paste0("./note_04_random/jpg/note_04_random-", seq(1,5,1),".jpg")
